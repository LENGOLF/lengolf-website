...
export default async function Image({ params }: Props) {
  ...
  // The same roster calls the page makes (8 nearest), so the count, the
  // distance and the noun match what it lists.
  ...
  if (airport) {
    ...
    return ogCard({
      ...
      chips: [`${items.length} nearest courses`, `Closest ~${items[0].km.toFixed(1)} km`],
