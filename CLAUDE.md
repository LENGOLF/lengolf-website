...
- **Emoji detection is `\p{Emoji_Presentation}`, not `\p{Extended_Pictographic}`.** The latter is much wider than "emoji" in the sense the glossary tone means: it also covers text-presentation symbols that render as ordinary glyphs. Adding the `messages/*.json` corpus proved it — `Extended_Pictographic` flagged the `©` in `© {year} LENGOLF CO., LTD.` (a legal mark, identical in the English source) and the `★` in `★{rating} / Googleレビュー{count}件` (a rating glyph in the trust chip). Neither was a defect; the regex was. Real emoji (🎉/⛳/😀) still fail, and a text symbol explicitly emoji-styled with VS16 (`⚠️`, `✔️`) still fails — that IS decoration. Self-test locks both directions.
...
- **Validate course fee plausibility:** `npm run validate:courses` — lints `data/golf-courses/*/*.ts` green fees. ERROR (fails CI): fee outside ฿150–฿20,000, weekend cheaper than weekday, or a suspicious pattern (weekday under ฿600 — the classic scraped Thai-national/9-hole/promo rate — or weekend >2.2× weekday) on a course with no `fees_verified_at` attestation. WARN: attested-but-suspicious fees, attestations older than 18 months. When correcting a fee, also grep the course's `prose` for the old number — fees are often repeated in prose text. No server needed.
...
- **`build-and-smoke`** — builds the app, starts the production server, runs smoke tests across 16 categories (per-category test counts live in `scripts/smoke-test.ts`):
  ...
  - **M) Wayfinding copy** — BTS Chidlom is **Exit 4** (The Mercury Ville). Asserted on the six DB-driven `/location/*-chidlom` pages (`location_pages.bts_route`, unreachable by any static lint) and on the repo-owned strings: `HomeJa/HomeKo/HomeZh.accessBts` in `messages/*.json` and the hardcoded EN line in `app/[locale]/golf-in-thailand-guide/page.tsx`. Each check is a matched pair — correct string present *and* wrong-exit shape absent — so dropped copy fails rather than passing vacuously. Assertions run against rendered markup with `<script>` blocks stripped: `NextIntlClientProvider` is handed the whole locale catalog, so an un-stripped body would let a string that never renders satisfy the check.
