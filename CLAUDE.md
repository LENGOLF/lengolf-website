...
- **⚠️ EVERY translation batch needs NATIVE-LANGUAGE QA before it merges — the
  ...
  **`/hotels` (48 content entries + 35 UI-chrome strings) shipped in PR #90
  ...
  a green gate alone. (The #90 review pass added `/hotels` to the
  `validate:i18n` corpus, where it had been absent — so it now gets the
  mechanical checks it was missing. That closes the emoji/terminology/casing
  hole; it is still not a prose read.)
