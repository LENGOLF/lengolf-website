...
- **A locale-aware component can be locale-correct in every string but one, and the one is whatever is built in CODE rather than read from the catalog.** `RoundupList` took `useLocale()` for `courseDetailHref` and still rendered the EN `prose.overview` and an EN `driveTimeLabel()` beside it on `/{th,ja,ko,zh}/golf-courses/under/<tier>/` (PR #97). The tell is a component that calls `useTranslations` **and** also calls a formatter or reads a `prose`/`locales` field: `t(...)` cannot be wrong, so the audit eye slides past the two calls that can. Three consequences. (a) The per-field EN fallback for course prose is now ONE function, `localizedCourseProse` in `lib/course-seo.ts` — route new call sites through it. It deliberately omits `rental_cta_context`, which wants `undefined` on an untranslated locale so `RentalCtaBanner` renders its own localized default; the `Omit<>` return type makes folding it back in a compile error. (b) `toFormatLocale` (`lib/format.ts`) exists so CLIENT components can narrow a locale without importing `course-seo` and dragging `FAQ_L10N` into the browser bundle — verified by grepping `.next/static/chunks`, where a `FAQ_L10N`-unique ko template is absent while a `lib/format.ts` string reached via the same client component is present. (c) Do NOT collapse the raw locale into the content-locale union before passing it to `courseDetailHref` — that takes the ROUTING locale, and prefixing is a per-course decision, not a per-locale one. **Scope fence:** the sweep behind this was a repo-wide grep of `prose.overview` and `driveTimeLabel(` on 2026-08-16, and it dispositioned exactly those two symbols. It is NOT a proof that every localized surface is locale-correct — `course.province` is the standing counterexample (next bullet).
- **`course.province` still renders raw ENGLISH on every translated course surface, and it is a translation gap, not a code gap.** `RoundupList.tsx`, `CourseMapExplorer` (info panel + roster), `CoursePage` (hero badge + related cards) and `addressLocality` in the GolfCourse JSON-LD all print `c.province` unlocalized — 49 English province spans per translated hub, so `/ko/golf-courses/bangkok/nikanti-golf-club/` shows `나콘빠톰` in its `<title>` and **`Nakhon Pathom`** in the hero badge, two spellings of one province on one page (the same failure class as the 齐隆/奇隆 bug). `PROVINCE_L10N` (`lib/course-seo.ts`) is module-private and, critically, **covers only registered-course provinces: 17 provinces spanning 53 courses have no entry at all.** So localizing only the mapped ones would make a `/under/<tier>/` roster — which spans regions — mix scripts. Closing this needs a 68-string batch (17 × 4 locales) with native QA plus an exported accessor, not a one-line render fix. Do not "quickly localize the province" without that batch.
- **Which characters end a sentence is a per-language fact; a universal splitter is a bug in at least two of the five locales.** `firstSentence()` in `RoundupList` matched `/^[^.!?]*[.!?]/`. Measured across all 149 courses (2026-08-16): **ja 41/50 and zh 41/50 found no ASCII terminator at all** (they end sentences with `。`) and returned the whole paragraph; **ko 50/50 split correctly on an ASCII period**, which Korean genuinely uses; and **Thai has no sentence-terminating punctuation whatsoever**, so the first stray `.` in a Thai overview is a decimal, a clock time (`06.00`), or an abbreviation (`น.`, `Dr.`, `MBK Public Co.`) sitting hundreds of characters in. It shipped th/zh pull quotes reading `…ออกแบบโดย Dr.` and `…与Japan Golf Promotion Co.` — the latter leaving a Chinese clause with no verb, because `合作创办` was in the discarded tail. The function takes the locale now: ja/zh split only on `。`, th does not split. **The tempting regex fix does NOT work** — exempting a decimal point (`(?:[^.!?。！？]|(?<=\d)\.(?=\d))*[.!?。！？]`) BACKTRACKS and re-matches that same `.` when no later terminator exists, so it appears to work on the one entry that has a later ` น.` to reach and silently reproduces the cut everywhere else. Known gaps kept deliberately: EN and ko still cut at Latin abbreviations (pre-existing; fixing it would move EN output), and the Thai `<p>` now carries the entire ~795-char overview inside a `line-clamp-2` — invisible to sighted readers, read in full by screen readers and crawlers. A real Thai pull quote needs an authored summary field, not a smarter splitter.
- **Smoke section O is the only gate that can see a missing locale argument — nothing else can.** Mutation testing on PR #97 ran the original bug plus four separate reverts of the fix against `tsc`, `lint`, `validate:links`, `validate:i18n`, `validate:courses`, `validate:hotels` and `validate:fee-labels`: **35 measurements, 35 green.** Two structural reasons, both permanent: `driveTimeLabel(min, withSuffix, locale = 'en')` gives the locale a DEFAULT value, so omitting it is type-legal; and every data validator imports zero files from `components/`, so none can observe a rendering decision. Section O fetches all 76 registry-derived translated hub + tier URLs and asserts no page carries the EN template shape `/~\d+(min|h)/`. Two design points worth keeping. The negative half is safe **because `~50 min` is a code-generated format literal, not prose** — it carries none of the false-positive risk of the `checkScript` cautionary tale above, which fired on eleven legitimately cross-script strings. The positive half is a **section-level** marker floor, not per-page, because `/{th,ja,ko,zh}/golf-courses/{phuket,chiang-mai}/` legitimately render no drive time at all (no course there carries the field) and a per-page matched pair would fire on eight correct pages. Red-tested by removing the locale argument: 8/8 sampled pages leaked and localized markers dropped to 0.
