...
- **No `validate:*` script can catch a rendering bug — every one of them imports zero files from `components/`.** A wrong-locale or wrong-label *render* is only observable by fetching the page, so it needs a smoke section, not a lint rule. Compounding it, an optional parameter with a default (`driveTimeLabel(min, withSuffix, locale = 'en')`) makes the omission type-legal, so `tsc` is blind too. Precedent: PR #97's missing locale argument survived `tsc`, `lint` and all five validators across **35 measurements, 35 green** — the original bug plus four separate reverts of the fix. That is what smoke section O exists for; design notes live next to it in `scripts/smoke-test.ts`.
...
- **`build-and-smoke`** — builds the app, starts the production server, runs smoke tests across 22 categories (per-category test counts live in `scripts/smoke-test.ts`):
  ...
  **F2 and L5 were live in `scripts/smoke-test.ts` but missing from this list before PR #98, so the count above read "19" when `main()` already called 21 runners. If you add a runner to `main()`, add its line here in the same commit.**
