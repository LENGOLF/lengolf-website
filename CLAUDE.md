...
- **⚠️ EVERY translation batch needs NATIVE-LANGUAGE QA before it merges — the
  ...
  Native QA has caught a blocker in **five of six** batches run so far, and none
  ...
  **since had its native prose read** (the batch that caught the ko particle
  blocker above, plus a few grounded nits). Debt discharged: every SEO section
  in the corpus has now been read as prose by a native speaker. Keep the rule
...
- **A typed fee field can disagree with every locale's prose, and the FAQ generator will print both.** `phuket-country-club` carries `green_fee_weekday_thb: 2800` / `green_fee_weekend_thb: 4000`, but the EN meta and the th/ja/ko/zh prose all frame those same two numbers as **low season / high season**. `validate:courses` passes (weekend > weekday, both plausible) and nothing cross-checks prose against field semantics. Because `FAQ_L10N.feeAnswer` builds the fee FAQ from the TYPED fields, the page renders "low season / high season" in its body next to an auto-generated FAQ saying "weekday / weekend" — and the FAQ copy also ships as JSON-LD `FAQPage` structured data. **Resolved, but only after three separate rounds of "all consumers now honor it" turned out to be false.** The course carries `fee_is_seasonal: true`, and the label decision is now centralized in **`lib/course-fees.ts`** (`pricesByDayOfWeek` / `feeLabelKeys` / `feeLabelsEn` / `feeBasisNoteEn`) — **route every new label site through it rather than writing another `fee_is_seasonal ?` ternary.** The lesson is not "remember to check the flag", it is: **a boolean like this is a cross-cutting content decision, and duplicating it per call site guarantees a miss.** The misses, in order: shipped wired into only the FAQ generator (fee table + JSON-LD `Offer` still claimed weekday/weekend); then "fixed everywhere" while `whenToChoose` in the compare route still emitted *"costs less on Saturday/Sunday"* for the seasonal course on an **indexed** URL; then "fixed everywhere" again while `data/golf-courses-use-cases.ts` still listed it under `/best-for/weekday-play/` with *"Weekday 2,800 vs weekend 4,000 — saves 1,200 THB"*, `under/[tier]` rendered *"Weekday from 2,800 THB"*, the region-hub roster tagged the higher fee `wknd`, and `SpecTable` had patched the fee **value** but left the row **label** reading "Weekday green fee" next to a cell reading "(low season)". **When auditing, grep `green_fee_weekday_thb` with NO directory scoping** — `data/golf-courses-use-cases.ts` lives outside `lib/ components/ app/` and was missed for exactly that reason; and note that a shared column header (compare, near-station) can only name a basis when every course in the table agrees, otherwise the header goes neutral and each cell carries its own basis. When adding a locale to a course, still check the typed fields mean what the prose says. **Sibling pattern — `caddie_tip_included`:** Nikanti's all-inclusive green fee covers the caddie tip, but `FAQ_L10N.caddieAnswer` hard-appended "Caddie tips … are customary on top" for *every* mandatory-caddie course — contradicting the course's own copy, in `FAQPage` structured data. A `caddie_tip_included` flag (set on Nikanti, threaded through `caddieAnswer(name, required, fee, tipIncluded)`) now switches that clause to "the tip is already included — no extra tipping expected." Do NOT key seasonal-or-tip-inclusive behavior off `caddie_fee_thb: 0` or a bare fee value — eight courses carry `caddie_fee_thb: 0` without being all-inclusive; use the explicit flag.
