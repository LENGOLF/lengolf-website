...
export const course: GolfCourse = {
  ...
  locales: {
    ...
    ko: {
      title: `Laem Chabang International Country Club 그린피 — 촌부리 27홀 코스 가이드`,
      meta_description: `Laem Chabang International Country Club은 Si Racha에 자리한 잭 니클라우스 설계의 27홀 코스예요. 요금은 그린피·캐디·카트가 포함된 올인클루시브이고, 야간 골프도 인기예요. 방콕 호텔로 배달되는 LENGOLF 클럽 대여도 안내해 드려요.`,
      prose: {
        overview: `Laem Chabang International Country Club은 촌부리주 Si Racha의 변화무쌍한 700에이커 부지에 펼쳐진 27홀 잭 니클라우스 설계 코스로, 1990년대 초 태국에서 완성된 여섯 개의 니클라우스 설계 코스 중 하나예요. 개장은 1993년이고요. 코스 이름은 인근의 Laem Chabang 심해항에서 따왔는데, 이 항구는 동남아시아에서 가장 붐비는 컨테이너항 중 하나로 꼽혀요. 세 개의 9홀 루프는 각각이 지나가는 서로 다른 환경에 따라 Lake, Mountain, Valley로 이름 붙었고, 9홀마다 코스의 성격이 뚜렷하게 바뀌어요. 15:30 무렵부터 즐길 수 있는 야간 골프도 마련돼 있어, 한낮의 더위를 피하고 싶은 파타야 거주 골퍼에게 인기예요. 올인클루시브 요금에는 그린피·캐디·카트가 하나로 포함돼요.`,
    ...
    zh: {
      ...
      prose: {
        overview: `Laem Chabang International Country Club是一座27洞的杰克·尼克劳斯（Jack Nicklaus）设计球场，铺展在春武里府Si Racha面积700英亩、地貌多变的场地上，也是1990年代初在泰国建成的六座Nicklaus球场之一。球场于1993年开放，名字取自附近的Laem Chabang深水港——东南亚最繁忙的集装箱港口之一。三组九洞按各自穿行的不同环境命名为Lake、Mountain与Valley，每打完九洞，球场的性格就会明显换一副面孔。约15:30起可以享受夜间高尔夫，很受想避开白天暑热的芭提雅本地球友欢迎。全包价格把果岭费、球童与球车统一包含在内。`,
