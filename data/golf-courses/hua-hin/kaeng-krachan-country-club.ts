...
export const course: GolfCourse = {
  ...
  locales: {
    ...
    ko: {
      ...
      prose: {
        overview: `Kaeng Krachan Country Club & Resort 코스는 후아힌 일대에서 경관이 가장 극적인 골프 목적지 가운데 하나예요. 후아힌에서 내륙으로 51km 들어간 Kaeng Krachan 국립공원 가장자리에 자리하고 있고, 이 국립공원은 유네스코 세계유산이에요. 27홀 규모의 이 복합 코스는 태국인 설계가 Cherd Bunyaratavej의 원래 18홀 레이아웃(1994년 개장)과 2008년에 더해진 Jack Nicklaus Mountain Course 9홀을 함께 갖추고 있습니다. 둘을 합치면 탁 트인 파크랜드부터 빽빽한 정글 지형까지 폭넓게 아우르는 시설이 되고, 특히 Nicklaus 나인은 이 가격대에서는 좀처럼 만나기 어려운 극적인 고저 차와 숲의 프레이밍을 보여 줘요. 2023년부터는 Power Nine Group에서 코스를 운영하면서 평일 1,199바트, 주말 1,399바트부터 시작하는 올인클루시브 패키지를 내놓고 있어요. 캐디와 공유 카트가 요금에 포함됩니다. 진짜 Nicklaus 브랜드를 단 9홀과 빼어난 자연 환경, 그리고 부담 없는 가격이 어우러져, 내륙까지 들어갈 마음이 있는 골퍼에게 Kaeng Krachan 코스는 남다른 목적지가 돼요.`,
        layout_and_experience: `Kaeng Krachan에서 Cherd Bunyaratavej의 원래 설계는 Lake Course, Valley Course 두 나인으로 나뉘어, 부지에서 비교적 트여 있고 파크랜드에 가까운 구역을 함께 아우릅니다. 이 두 나인은 오래 자란 나무가 둘러싼 넉넉한 페어웨이가 특징이고, Lake 나인의 워터 해저드가 전략적으로 가장 큰 과제예요. Valley Course 코스는 더 빽빽한 식생과 미묘한 고저 차 사이를 누비며, 절제된 어프로치에 보답합니다. 세 번째이자 가장 개성이 뚜렷한 나인인 Jack Nicklaus Mountain Course 코스는 지형과 드라마 양쪽에서 한 단계 도약을 보여 줘요. Nicklaus 설계팀은 산비탈의 등고선을 그대로 살려 울창한 정글 사이를 오르내리는 홀을 만들었고, 국립공원과 아래쪽 저수지를 굽어보는 조망이 펼쳐집니다. 여러 홀에서 티샷은 빽빽한 숲을 넘겨야 하고, 그린은 높이 솟은 대지 위에 얹혀 있어 어프로치 거리 계산이 결정적이에요. Mountain Course 라운딩 중에 사슴과 왕도마뱀, 여러 종류의 새 같은 야생동물을 마주치는 일도 드물지 않습니다. 세 나인을 조합하면 하루 플레이 안에서도 상당한 변화를 누릴 수 있어요.`,
